...
import signal
import pathlib
import easygui #AI tk代码难以维护，最近不打算懂了，用这个库来做一些简单的交互界面，后续如果有时间再重写一个tk版本的界面
import time
# ---------- 全局变量 ----------
LOCK_FILE = 'app.lock'
pids = psutil.pids()
home_dir = str(pathlib.Path.home())#严格来说应该放在函数里，但反正这个变量也不大，放在这里也无所谓
running = False
worker_thread = None
time_interval = 0.1          
input_length = 10            
...
def remove_lock_file():
    """删除锁文件"""
    if os.path.exists(home_dir+"\\"+LOCK_FILE):
        os.remove(home_dir+"\\"+LOCK_FILE)
def check_single_instance():
    """检查是否已有实例在运行"""
    # 建议在函数内获取最新的 PID 列表，而不是使用全局过期的列表
    current_pids = psutil.pids() 
    lock_path = os.path.join(home_dir, LOCK_FILE)
    
    if os.path.exists(lock_path):
        try:
            # 使用 with 语句确保读取后自动关闭句柄，防止解除占用失败
            with open(lock_path, 'r', encoding='utf-8') as f:
                content = f.read().strip()
            
            # 将读取到的字符串转换为整数进行对比
            if content and int(content) in current_pids:
                print("程序已在运行，请勿重复启动。")
                easygui.msgbox("\n\n                        程序已在运行，请勿重复启动。", "提示")
                sys.exit(1) 
            else:
                # 锁文件存在但进程不在列表中，说明是残留文件，直接删除[cite: 2]
                os.remove(lock_path)
        except (ValueError, PermissionError, Exception):
            # 如果文件损坏或被其他进程锁定无法删除，尝试直接报错或跳过
            pass

    # 如果没有运行中的实例，则创建锁文件并写入当前进程 PID[cite: 1, 2]
    try:
...
def run_gui():
    ...
    def on_dict_mode_changed():
        ...
        if not enable_dictionary_mode:
            ...
            dict_list_label.config(text=f"字典列表: ({len(dictionary_files)} 个)", foreground="green")
    ...
    dict_display_frame.grid(row=8, column=0, columnspan=2, sticky=tk.NSEW, pady=(5, 5))
    ...
    dict_scrollable_frame.bind("<Configure>", lambda e: dict_canvas.configure(scrollregion=dict_canvas.bbox("all")))
    ...
    def update_dict_display():
        ...
        if not dictionary_files:
            ttk.Label(dict_scrollable_frame, text="(暂无字典)", foreground="gray").pack(anchor=tk.W, padx=5, pady=5)
        else:
            for fpath in dictionary_files:
                item_frame = ttk.Frame(dict_scrollable_frame)
                item_frame.pack(anchor=tk.W, fill=tk.X, expand=True, padx=3, pady=3)
                
                is_enabled = dictionary_enabled.get(fpath, True)
                enable_var = tk.BooleanVar(value=is_enabled)
                
                def make_toggle(fp, var):
                    return lambda: (dictionary_enabled.update({fp: var.get()}), save_config(), log_info(f"字典 {os.path.basename(fp)} 已切换"))
                
                ttk.Checkbutton(item_frame, text=os.path.basename(fpath), variable=enable_var, command=make_toggle(fpath, enable_var)).pack(side=tk.LEFT)
                
                def make_delete(fp):
                    def delete():
                        if messagebox.askyesno("确认", f"删除字典: {os.path.basename(fp)}?"):
                            dictionary_files.remove(fp)
                            dictionary_pools.pop(fp, None)
                            dictionary_enabled.pop(fp, None)
                            update_dict_display()
                            save_config()
                    return delete
                
                ttk.Button(item_frame, text="删除", width=5, command=make_delete(fpath)).pack(side=tk.RIGHT)

    def select_dictionary():
        from tkinter import filedialog
        filepath = filedialog.askopenfilename(filetypes=[("文本文件", "*.txt")])
        if filepath and load_dictionary(filepath):
            update_dict_display()
            save_config()

    # --- 3. 字典操作按钮 (Row 9) ---
    dict_btn_frame = ttk.Frame(main_frame)
    dict_btn_frame.grid(row=9, column=0, columnspan=2, sticky=tk.EW, pady=5)
    ttk.Button(dict_btn_frame, text="添加字典", command=select_dictionary).pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)
    ttk.Button(dict_btn_frame, text="清空所有字典", command=lambda: (dictionary_files.clear(), dictionary_pools.clear(), update_dict_display(), save_config())).pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)
    ...
    def apply_config():
        ...
        except: messagebox.showerror("错误", "输入格式有误")

    def delete_logs():
        if messagebox.askyesno("确认", "清空所有本地日志文件？"):
            try:
                for f in os.listdir(LOG_DIR):
                    if f.endswith(".log"): os.remove(os.path.join(LOG_DIR, f))
                log_info("历史日志已清理")
            except Exception as e: log_info(f"清理失败: {e}")

    config_btn_frame = ttk.Frame(main_frame)
    config_btn_frame.grid(row=10, column=0, columnspan=2, sticky=tk.EW, pady=10)
    ttk.Button(config_btn_frame, text="保存并应用当前配置", command=apply_config).pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)
    ttk.Button(config_btn_frame, text="删除历史日志文件", command=delete_logs).pack(side=tk.LEFT, padx=5, expand=True, fill=tk.X)
    ...
    control_frame.grid(row=13, column=0, columnspan=2, sticky=tk.EW, pady=5)
    ttk.Button(control_frame, text="启动监控", command=start_worker).pack(side=tk.LEFT, padx=20, expand=True, fill=tk.X)
    ttk.Button(control_frame, text="停止监控", command=stop_worker).pack(side=tk.LEFT, padx=20, expand=True, fill=tk.X)

    log_header = ttk.Frame(main_frame)
    log_header.grid(row=14, column=0, columnspan=2, sticky=tk.W, pady=(10, 2))
    ttk.Label(log_header, text="运行日志:").pack(side=tk.LEFT)
    ttk.Button(log_header, text="清空显示", command=clear_logs, width=10).pack(side=tk.LEFT, padx=10)
    ...
    ttk.Label(main_frame, text="快捷键: Ctrl + Alt + Q 切换开关", foreground="gray").grid(row=16, column=0, columnspan=2, pady=10)
...
if __name__ == "__main__":
    ...
    run_gui()
    remove_lock_file()#如果应用正常跑完，这个函数就会被调用，反之不正常退出会出现锁文件残留的情况，导致下次无法启动，所以在开头加了检测自身是否退出的code
